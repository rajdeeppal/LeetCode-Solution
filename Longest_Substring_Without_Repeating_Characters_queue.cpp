...
bool check_post(std::map<char, std::queue <int> > v , char x){
auto itr = v.find(x);
...
int check_string( std::map<char, std::queue <int> >& trace , char x){
auto itr = trace.find(x);
//auto it = itr->second.begin();

int p = itr->second.front() + 1 ;
std::cout << "new i "<< p << '\n';
...
int main(int argc, char const *argv[]) {
  ...
  //std::cout << "/* message */" << '\n';
  std::map<char, std::queue< int > > trace;
  int i = 0;
  int j = 0;
  int maxi = -1 ;
  while ( i < len && j < len) {
